...
use App\Model\{Firm,BankDetails,Role,CompanyInformation,Employees,Customer,TempCustomers,PublicHolidays,PaymentBatches};
...
class SalaryBatchController extends Controller
{
    ...
    {     
        ...
        return view('admin.salary-batch.pending',compact('pagename'));
    ...
    private function dataTableColumnBindings(){
        $columns = array(
            ...
            array( 'dbAlias'=>'payment_batches','db' => 'batch_service_type', 'dt' => 2,
                    'formatter' => function( $d, $row ) {
                       return ucfirst($row['batch_service_type']);
                    }
                ),
            array( 'dbAlias'=>'payment_batches','db' => 'payment_date',  'dt' => 3,
                    'formatter' => function( $d, $row ) {
                    return Helper::convertDate($row['payment_date'],'d-m-Y');
                    }
             ),
            array( 'dbAlias'=>'payment_batches','db' => 'created_on',  'dt' =>4,
                    'formatter' => function( $d, $row ) {
    ...
    {     
        ...
        return view('admin.salary-batch.processed',compact('pagename'));
    ...
    {     
        ...
        return view('admin.salary-batch.queued',compact('pagename'));
    ...
    public function ajaxQueuedList(Request $request){
        ...
        $bindings=['salary','approved','sent',1];
