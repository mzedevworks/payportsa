...
function generateAction(data, type, row, meta){
    ...
    var str='<div class="float-left"><a href="{{url('merchant/collection/normalbatch/approved-transmission/transaction/')}}/'+data+'" class="btn btn-common mr-2">Collections</a></div>';
...
function formatBatchName(data, type, row, meta){
    ...
    var str='<a href="{{url('merchant/collection/normalbatch/approved-transmission/transaction/')}}/'+data.id+'">'+data.batch_name+'</a>';
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        buttons: [
            //'copy', 'excel', 'pdf', 'print'
            {
                    ...
                    filename:"batch-submitted"
                }
        ],
        ajax: {
            url: "{{ url('merchant/collection/normalbatch/ajax-approved-list')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
