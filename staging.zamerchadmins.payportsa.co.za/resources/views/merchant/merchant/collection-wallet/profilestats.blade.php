          ...
          <div class="row">
            <div class="col-12">
              <div class="d-flex no-block align-items-center">
                <div>
                   <div class="icon"><i class="lni-empty-file"></i></div>
                   <p class="text-muted">Line Limit</p>
                </div>
                <div class="ml-auto">
                   <h2 class="counter text-info">
                   {{$profileLimits->line_collection}}
                   </h2>
                </div>
              </div>
            </div>
          ...
          <div class="row">
            <div class="col-12">
              <div class="d-flex no-block align-items-center">
                <div>
                   <div class="icon"><i class="lni-empty-file"></i></div>
                   <p class="text-muted">Surety %</p>
                </div>
                <div class="ml-auto">
                   <h2 class="counter text-info">
                   {{$profileLimits->surety_amount}}
                   </h2>
                </div>
              </div>
            </div>
            ...
            <table id="firms-datatable" class="table table-bordered">
              ...
              <tbody>
                @if(count($ledgerData)>0)
                  @foreach($ledgerData as $eachStatement)
                    <tr>
                      <td>{{$eachStatement['desc']}}</td>
                      <td>{{$eachStatement['amount']}}</th>
                      <td>{{$eachStatement['closing_balance']}}</td>
                      <td>{{Helper::convertDate($eachStatement['trnx_date'],'d-m-Y')}}</td>
                    </tr>
                  @endforeach
                @endif
              </tbody>
