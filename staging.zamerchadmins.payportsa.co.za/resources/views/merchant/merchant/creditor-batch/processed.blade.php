...
function generateAction(data, type, row, meta){
    ...
    var str='<div class="float-left"><a href="{{url('merchant/employees/batch/processed-transaction')}}/'+data+'" class="btn btn-common mr-2" data-toggle="tooltip" data-placement="top" data-original-title="Details">Payments</a></div>';
...
function formatBatchName(data, type, row, meta){
    ...
    var str='<a href="{{url('merchant/employees/batch/processed-transaction/')}}/'+data.id+'" data-toggle="tooltip" data-placement="top" data-original-title="Details">'+data.batch_name+'</a>';
...
$(document).ready(function() {
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        ajax: {
            url: "{{ url('merchant/employees/batch/ajax-processed')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
