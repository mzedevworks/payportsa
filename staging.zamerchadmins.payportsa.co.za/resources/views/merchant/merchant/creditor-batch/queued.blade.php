...
function generateAction(data, type, row, meta){
    ...
    var str='<div class="float-left"><a href="{{url('merchant/creditors/batch/queued-transaction')}}/'+data+'" class="btn btn-common mr-2">Payments</a></div>';
...
function formatBatchName(data, type, row, meta){
    ...
    var str='<a href="{{url('merchant/creditors/batch/queued-transaction')}}/'+data.id+'"  data-toggle="tooltip" data-placement="top" data-original-title="Details">'+data.batch_name+'</a>';
...
$(document).ready(function() {
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        ajax: {
            url: "{{ url('merchant/creditors/batch/ajax-queued')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
