        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom">
                    ...
                    <form method="POST" action="{{url('merchant/employees/savebatch')}}" class="btn bg-transparent" id="approveStatusForm">
                        {{csrf_field()}}
                        {{method_field('POST')}}
                        ...
                        <input type="hidden" name="employee_selection" value="{{$postData['employee_selection']}}"/>
...
function confirmFormSubmit(message,formId){
    if($("#employeeList").length>0){
        $("#employeeList").val(JSON.stringify(selectedCustomers));
    }else{
        $("#batchContainerDiv").append("<input type='hidden' id='employeeList' name='employeeList' value='"+JSON.stringify(selectedCustomers)+"'>");
    }
    

    if($("#employeeAmount").length>0){
        $("#employeeAmount").val(JSON.stringify(selectedAmount));
    }else{
        $("#batchContainerDiv").append("<input type='hidden' id='employeeAmount' name='employeeAmount' value='"+JSON.stringify(selectedAmount)+"'>");
    }

    if($("#employeeReff").length>0){
        $("#employeeReff").val(JSON.stringify(selectedReffrence));
    }else{
        $("#batchContainerDiv").append("<input type='hidden' id='employeeReff'name='employeeReff' value='"+JSON.stringify(selectedReffrence)+"'>");
    }

    if($("#salaryBatchAmount").length>0){
        $("#salaryBatchAmount").val(totalBatchAmount);
    }else{
        $("#batchContainerDiv").append("<input type='hidden' id='salaryBatchAmount' name='salaryBatchAmount' value='"+totalBatchAmount+"'>");
    }
...
function createAmountInput(data, type, row, meta){
    var str='<input data-id="'+row[0]+'" id="amount_'+row[0]+'" type="text" class="form-control" placeholder="Amount" name="amount" value="'+data+'" onchange="updateEmployee(this)">';
    
    return str;
}

function createReffrenceInput(data, type, row, meta){
    var str='<input data-id="'+row[0]+'" id="refference_'+row[0]+'" type="text" class="form-control" placeholder="reffrence" name="reffrence" value="'+data+'"  onchange="updateEmployee(this)">';
    
    return str;
}

function updateEmployee(elem){
    var employeeId=$(elem).data('id');
    var elemVal=$("#checkUsers_"+employeeId).val();
    var arrIndex=selectedCustomers.indexOf(elemVal);

    if($("#checkUsers_"+employeeId).prop("checked") == true){
        
        
        if(arrIndex!==-1){
            totalBatchAmount=totalBatchAmount-selectedAmount[arrIndex];
            selectedCustomers[arrIndex]=elemVal;
            selectedAmount[arrIndex]=$("#amount_"+employeeId).val();
            selectedReffrence[arrIndex]=$("#refference_"+employeeId).val();
            totalBatchAmount=totalBatchAmount+selectedAmount[arrIndex];
        }
...
function customerClicked(elem){
    ...
    if($(elem).prop("checked") == true){
        if(arrIndex===-1){
            ...
            totalBatchAmount=totalBatchAmount+empSalary;
        }
...
$(document).ready(function() {
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        searching: true,
        ajax: {
            url: "{{ url('merchant/employees/listforbatch')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
