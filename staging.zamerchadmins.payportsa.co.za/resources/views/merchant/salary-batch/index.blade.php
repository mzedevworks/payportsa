...
function generateAction(data, type, row, meta){
    //var str='<div class=""><a href="{{url('merchant/employees/update-batch/')}}/'+data+'" class="m-1 bg-transparent"><i class="lni-pencil" aria-hidden="true"></i></a>';
    var str='<div class=""><a href="{{url('merchant/employees/batch/pending-transaction/')}}/'+data+'" class="m-1 bg-transparent" data-toggle="tooltip" data-placement="top" data-original-title="Edit"><i class="lni-pencil" aria-hidden="true"></i></a>';
    str+='<a href="{{url('merchant/employees/batch/pending-tranx-detail/')}}/'+data+'" class="m-1 bg-transparent" data-toggle="tooltip" data-placement="top" data-original-title="View Details"><i class="lni-eye" aria-hidden="true"></i></a>';
...
function formatBatchName(data, type, row, meta){
    ...
    var str='<a href="{{url('merchant/employees/batch/pending-transaction/')}}/'+data.id+'">'+data.batch_name+'</a>';
    ...
    confirmDialog("Are you sure to "+actionType+" this record?", (ans) => {
        if (ans) {
            $.ajax({
                ...
                type  : 'post',
                url   : "{{url('merchant/employees/batch/statusupdate')}}",
...
$(document).ready(function() {
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        ajax: {
            url: "{{ url('merchant/employees/batch/ajax-pending')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
    ...
    $('#customers-datatable_filter input').unbind('keypress keyup').bind('keypress keyup', function(e){
        ...
        myCustomerTable.search($(this).val()).draw();
        //fnFilter($(this).val());
    });
