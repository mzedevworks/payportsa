...
use PDO;
use App\Model\{Firm,BankDetails,PublicHolidays,ProfileTransactions};
...
class Helper
{
    ...
    public static function getNextCollectionDate($date,$debit_frequency,$lastDay=false){
        ...
        if($debit_frequency=="Weekly"){
            $next_collection_date = date('Y-m-d',strtotime("+1 week",strtotime($date)));
        }
        if($debit_frequency=="Monthly"){
            ...
            if(($dayInCurrMonth>$daysNextMonth && $dayOfMonth>$daysNextMonth) || $lastDay==true){
                ...
                $next_collection_date = $nextYear."-".str_pad($nextMonth, 2, "0", STR_PAD_LEFT)."-".str_pad($daysNextMonth, 2, "0", STR_PAD_LEFT).'sss';
            }else{
                $next_collection_date = date('Y-m-d',strtotime("+1 months",strtotime($date)));
            }
            //$next_collection_date = date('Y-m-d',strtotime("+1 months",strtotime($date)));
        ...
        if($debit_frequency=="Annually"){
            ...
            if($dayInCurrMonth>$daysNextMonth || $lastDay==true){
                ...
                $next_collection_date = date('Y-m-d',strtotime("+1 year",strtotime($date)));
            }
        }
        if($debit_frequency=="Bi-annualy"){
            $next_collection_date = date('Y-m-d',strtotime("+6 months",strtotime($date)));
        }
    ...
    public static function getSftp(){
        ...
        // $ftp_server="home727248187.1and1-data.host";
        // $ftp_username = "u92825131";             
        // $ftp_userpass = "Admin!123";  

        $sftp = new SFTP($ftp_server);
        if (!$sftp->login($ftp_username, $ftp_userpass)) {
                return false;
            }
        //$sftp = new SSH2($ftp_server);
