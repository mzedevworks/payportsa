...
class SamedayPayReplydownloadCmd extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'transaction:download-sameday-payment-outputs';
     ...
     * @var string
     */
    protected $description = 'Download reply and output files of sameday payment from ABSA server';
    protected $dirName='';
    ...
    {
        parent::__construct();
        $this->dirName=public_path(Config('constants.localSameDayPaymentFileDownloadStoragePath'));
    }
    ...
    public function handle()
    {
        ...
        $remoreDir=Config('constants.remoteSameDayFileDownloadLocation');
        ...
        if($sftp){
            ...
            foreach($filesInDir as $key => $eachFile){
                ...
                }elseif($filePreName==Config('constants.sameDayPaymentSuitFolder')){
                    //download file
    ...
    */
    public function readdata() 
    {
        ...
        foreach($dir as $key => $data){
            if($key>1){ 
                $batchsequence = false;
                if(substr($data, 0, 8)==Config('constants.sameDayPaymentSuitFolder').'R')
                {
                    $this->readReplyFile($data);
                    //update   `collections` set transaction_status=1 where transmission_status=2 and transaction_status=2
                    
                }elseif(substr($data, 0, 8)==Config('constants.sameDayPaymentSuitFolder').'O'){
    ...
    function readReplyFile($fileName){
        ...
        $file = public_path(Config('constants.localSameDayPaymentFileDownloadStoragePath').$fileName);
        ...
        $transRecords=PaymentTransmissionRecords::whereNull('transmission_status')->where('transmission_type','sameday')->orderBy('id','desc')->first();
        ...
        $processedFilePath=Config('constants.localSameDayPaymentProcessedPath').$fileName;
    ...
    function readOutputFile($fileName){
        ...
        while(!feof($fn))  
        {
            ...
            if($recordIdentifier=="013"){
                ...
                ->where('payments.service_type','sameday')
    ...
    function rejectFullTranmission($lastTranmission){
        ...
        Payments::where('transmission_id', $lastTranmission->id)->where('service_type','sameday')->where('payment_status',1)->update(['transmission_status' => 3,'transaction_status'=>2]);
        $failedPayments=Payments::where('transmission_id', $lastTranmission->id)->where('payment_status',1)->where('service_type','sameday')->get();
        foreach($failedPayments as $eachFailedPayment){
            $this->refundPaymentLedger($eachFailedPayment);
        }
