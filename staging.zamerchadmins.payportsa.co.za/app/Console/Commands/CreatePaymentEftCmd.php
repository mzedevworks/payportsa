...
class CreatePaymentEftCmd extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'transaction:create-payment-eft';
     ...
     * @var string
     */
    protected $description = 'Create and upload transmission files for Payments EFT';
    ...
    public function __construct()
    {
        ...
        $this->localDir=Config('constants.localPaymentFileStoragePath');
        $this->remoteDir=Config('constants.remotePaymentOutputPath');
        $this->environment=Config('constants.payportEnv');
        $this->bankserUserCode = Config('constants.paymentbankSerUserCode');
        $this->electronicBankingSuitUserCode = Config('constants.paymentSuitUserCode');
        $this->electronicBankingSuitUserName = Config('constants.paymentSuitUserName');
    ...
    private function generateUserHeader($collectionRecords,$servType){
        ...
        foreach ($collectionRecords as $key => $eachCollection) {
            ...
            $accountType = isset($eachCollection->account_type) && $eachCollection->account_type== "Cheque" ? 1 : 2;
            $amountsum = $amountsum+$amount;

            $usersequencenumber    = sprintf('%06d', (intval($this->transactionCountOfToday)));

            $custAccountNameFillerLen  = 30-strlen($eachCollection->account_holder_name);
            $custAccountName=strtoupper($eachCollection->account_holder_name).str_repeat(' ',$custAccountNameFillerLen);
            ...
            */
            if($key!=0 || ($key+1)==sizeof($collectionRecords)){
                ...
                */
                if(($key+1)==sizeof($collectionRecords) || $lastFirmId!=$eachCollection->firm_id || $lastPaymentDate!=$eachCollection->payment_date){
    ...
    */
    ...
    private function fetchPaymentRecords(){
        $this->sameDayTransactions = $this->getRecords();
        $this->oneDayTransactions = $this->getRecords();
    ...
    function getPaymentDate($serviceTypeArr,$today){
        ...
        if(in_array("Same Day",$serviceTypeArr)){
            return $paymentDate;
        }elseif(in_array("1 Day",$serviceTypeArr) || in_array("2 Day",$serviceTypeArr)){
