...
class NormalBatchController extends Controller
{
    ...
    {     
        ...
        return view('admin.normal-batch.pending',compact('pagename'));
    ...
    private function dataTableColumnBindings(){
        $columns = array(
            ...
            array( 'dbAlias'=>'batches','db' => 'batch_service_type', 'dt' => 2,
                    'formatter' => function( $d, $row ) {
                       return ucfirst($row['batch_service_type']);
                    }
                ),
            array( 'dbAlias'=>'batches','db' => 'action_date',  'dt' => 3,
                    'formatter' => function( $d, $row ) {
                    return Helper::convertDate($row['action_date'],'d-m-Y');
                    }
             ),
            array( 'dbAlias'=>'batches','db' => 'created_at',  'dt' =>4,
                    'formatter' => function( $d, $row ) {
    ...
    public function ajaxPendingList(Request $request){
        ...
        $bindings=['normal-collection','pending'];
    ...
    {     
        ...
        return view('admin.normal-batch.processed',compact('pagename'));
    ...
    public function ajaxProcessedList(Request $request){
        ...
        $bindings=['normal-collection','processed',1];
    ...
    {     
        ...
        return view('admin.normal-batch.queued',compact('pagename'));
    ...
    public function ajaxQueuedList(Request $request){
        ...
        $bindings=['normal-collection','approved','sent',1];
        $whereConditions ="batches.batch_type =? and (batches.batch_status=? or batches.batch_status=?) and collections.collection_status=?";
        echo $this->getBatchData($request,$bindings,$whereConditions);
        die();
    }
