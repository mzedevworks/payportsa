...
function generateAction(data, type, row, meta){
    ...
    var str='<div class="float-left"><a href="{{url('merchant/collection/normalbatch/queued-transmission/transaction/')}}/'+data+'" class="btn btn-common mr-2">Collections</a></div>';
...
function formatBatchName(data, type, row, meta){
    ...
    var str='<a href="{{url('merchant/collection/normalbatch/queued-transmission/transaction/')}}/'+data.id+'">'+data.batch_name+'</a>';
...
$(document).ready(function() {
    ...
    myCustomerTable=$('#customers-datatable').DataTable( {
        ...
        ajax: {
            url: "{{ url('merchant/collection/normalbatch/ajax-submitted-list')}}"
        },
        //dom: "rtiS",
        "columns": [
            {
